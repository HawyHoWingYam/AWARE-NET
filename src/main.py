...
def train_all_models(config, transform):
    ...
    for dataset_name in datasets:
        ...
        train_df, val_df, test_df = create_data_splits(
            config, dataset_name, force_new=True
        )
        ...
        for model_key, model_config in models_config.items():
            ...
            variant_name = (
                f"{dataset_name}_{model_config['variant_key']}_no_augmentation"
            )
            ...
            train_dataset = DeepfakeDataset(
                dataframe=train_df, transform=transform, augment=False
            )

            val_dataset = DeepfakeDataset(dataframe=val_df, transform=transform)

            test_dataset = DeepfakeDataset(dataframe=test_df, transform=transform)
            ...
            if model_config["type"] == "single":
                model = SingleModelDetector(
                    model_key, config
                ).cuda()  # Pass model_key instead of timm_name
            else:
    ...
    for dataset_name in datasets:
        ...
        for model_key, model_config in models_config.items():
            # Use consistent variant naming
            variant_name = (
                f"{dataset_name}_{model_config['variant_key']}_with_augmentation"
            )
