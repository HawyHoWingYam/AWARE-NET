...
class Trainer:
    def __init__(
        self, model, train_loader, val_loader, config, variant_name, test_loader=None
    ):
        self.model = model
...
>>>>>>> main
    ...
    def save_model(self, epoch, val_loss):
        ...
        try:
            ...
            if hasattr(self.model, "dataset") and hasattr(self.model, "augment"):
                ...
                dataset = parts[0]
                variant = (
                    "with_aug" if "with_augmentation" in self.variant_name else "no_aug"
                )
            ...
            self.logger.error(
                f"Model key: {model_key if 'model_key' in locals() else 'unknown'}"
            )
    ...
    def train(self):
        ...
        for epoch in epoch_pbar:
            ...
            if val_loss < best_val_loss:
                ...
                self.logger.info(
                    f"New best validation loss: {val_loss:.4f} at epoch {epoch+1}"
                )
    ...
    def save_metrics(self):
        try:
            ...
            metrics_dir = (
                self.config.RESULTS_DIR / "metrics" / dataset / model_type / aug_type
            )
            ...
            self.logger.info(
                f"Current experiment is {'without' if is_no_aug else 'with'} augmentation"
            )
